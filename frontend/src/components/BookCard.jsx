...
const BookCard = ({ book, setBooks }) => {
  ...
  const handleDelete = async () => {
    ...
    return (
      <>
        {/* CARD */}
        <Link to={`/book/${book._id}`} className={`relative block rounded-xl bg-base-100 p-4 border transition-all duration-200${isActive ? "border-primary shadow-lg" : "border-base-300"}     hover:border-primary hover:shadow-x1`} >
          {/* Top Row */}
          ...
          {/* Footer */}
          <div className="mt-6 flex jsutify-between items-center">
        ...
        {showModal && (<dialog className="modal modal-open">
          <div className="modal-box">
            <h3 className="font-bold text-ig text-error flex items-center gap-2">
